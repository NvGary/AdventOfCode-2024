...
const _hasCorrectOrdering = (production: PrintQueue['production'][number], ordering: PrintQueue['ordering']): boolean => production.reduce<boolean>((acc, cur, idx) =>
...
export const hasCorrectOrdering = ({ ordering, production }: PrintQueue): PrintQueue['production'] =>
    ...
    production.filter(entry => _hasCorrectOrdering(entry, ordering));
...
const _correctOrdering = (production: PrintQueue['production'][number], ordering: PrintQueue['ordering']): PrintQueue['production'][number] => production.sort((a, b) => ordering.some(([l, r]) => a === l && b === r) ? -1 : ordering.some(([r, l]) => a === l && b === r) ? 1 : 0);

export const correctOrdering = ({ ordering, production }: PrintQueue): PrintQueue['production'] => production.filter(entry => _hasCorrectOrdering(_correctOrdering(entry, ordering), ordering));
